...
def login():
    if request.method == 'POST':
        ...
        """Check if user email exixts"""
        if user:
            """if user password correct"""
            if check_password_hash(user.password, password):
                flash('Logged in successfuly!', category='success')
                # kindda saves login session until user ...
                # clears browser or app restarts
                login_user(user, remember=True)
                return redirect(url_for('views.home'))
            else:
                flash('Incorrect password, try again.', category='error')
        else:
            flash('Email does not exixts', category='error')

    return render_template('login.html')

@auth.route('/logout')
@login_required # DECORATOR: cannot access logout unless user is logged in
def logout():
    logout_user()
